...
import { Text, View, FlatList, StyleSheet, Button, Dimensions } from 'react-native';
...
export default class RangeDatepicker extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.props.startDate != nextProps.startDate)
      this.setState({
        startDate: nextProps.startDate && moment(nextProps.startDate, 'YYYYMMDD'),
      });
    if (this.props.untilDate != nextProps.untilDate)
      this.setState({
        untilDate: nextProps.untilDate && moment(nextProps.untilDate, 'YYYYMMDD'),
      });
  }
  ...
  getInitialScrollIndex() {
    ...
    if (startDate && startDate != '') {
      ...
      const diff = startDate.diff(currentDate, 'months');

      if (diff >= 0 && diff < maxMonth - 1) return diff;
    }
      ...
      <View
        ...
        <FlatList
          ...
          getItemLayout={(data, index) => {
            return {
              length: this.props.monthContainerHeight,
              offset: this.props.monthContainerHeight * index,
              index,
            };
          }}
        />
